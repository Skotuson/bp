...
void Allocate::execute(WAMState &state)
{
    ...
    cp->localVariables().resize(m_N, nullptr);
    for (size_t i = 0; i < m_N; i++)
    {
        ...
        if (state.isQueryVariable(i) && state.SReg() == 1)
        {
            ...
            auto vw = std::make_shared<VariableWord>(&cp->localVariables()[i], state.getQueryVariableName(i));
            cp->localVariables()[i] = vw;
        ...
        else
        {
            ...
            cp->localVariables()[i] = std::make_shared<VariableWord>(&cp->localVariables()[i], "__" + std::to_string(state.getAllocatedVariables()));
